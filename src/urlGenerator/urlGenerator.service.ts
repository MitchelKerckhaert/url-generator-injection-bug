...
import { UrlGeneratorModuleOptions } from './urlGeneratorOptions.interface'
...
export class UrlGeneratorService {
  constructor(
    ...
    if (this.urlGeneratorModuleOptions.secret) {
      ...
      if (byteLength < 32) {
        Logger.warn('[urlGeneratorModuleOptions] The key size is recommended to be between 32-64 bytes')
      }
    }
  ...
  public signUrl({ relativePath, expirationDate, query, params }: SignUrlArgs): string {
    ...
    cloneQuery.signed = generateHmac(urlWithoutHash, this.urlGeneratorModuleOptions.secret)
  ...
  public isSignatureValid({ protocol, host, routePath, query }: IsSignatureValidArgs): boolean {
